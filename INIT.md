```lua
  require("lin-decompress"):setup({
	...
	-- Setting key, 'no_global_tar=false', to a .tar.* archive type appends these commands
	global_tar_compressor = {
		-- Commands for each .tar.* archive,
		-- Appends these 'cmd' only if 'no_global_tar = false' is set for a .tar.* configuration below
		cmd = { "-dkc" },
	},
